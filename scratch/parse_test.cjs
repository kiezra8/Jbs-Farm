...
wb.SheetNames.forEach(sheetName => {
  ...
  if (headerIndex !== -1) {
    ...
    for (let i = headerIndex + 1; i < Math.min(headerIndex + 15, rawRows.length); i++) {
      ...
      console.log(`Row ${i}: rDateVal=${rDateVal} (${typeof rDateVal}) -> rParsed=${rDateParsed} (lastReceipt=${lastReceiptDate}) | pDateVal=${pDateVal} (${typeof pDateVal}) -> pParsed=${pDateParsed} (lastPayment=${lastPaymentDate})`);
    }
  }
});
